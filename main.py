...
def DK_circle_magic(frame,magic_circle_cww,magic_circle_cw, lanmark, earthquake,count_earthquake, frame_cols, frame_rows):
    try:
        ...
        x1 = count_20[0]
        y1 = count_20[1]
        frame,deg = circle_magic(frame,magic_circle_cww, magic_circle_cw, x1, y1,deg, diameter)
        if count_earthquake % 3:
            earthquake[:, :-50] = frame[:,50:]
            frame = earthquake
        if count_earthquake % 5:
...
def DK_icon_neon(frame,hopden, dagiac, devil_mask, lanmark, frame_cols, frame_rows ):
    try:
        count_20 = _normalized_to_pixel_coordinates(lanmark.landmark[20].x,lanmark.landmark[20].y,frame_cols,frame_rows)
        kcmatna = caculate_distance(count_20[0], count_20[1], 150, 130)
        if kcmatna <100:
            frame = hinhkhoihop(frame, hopden, devil_mask,100,150)
            frame = cv.circle(frame, (232,127), 10, (255,0,0), -1)
            frame = cv.circle(frame, (275,127), 10, (255,0,0), -1)
            frame = neon_eye_matna(frame)
        else:
            frame = hinhkhoihop(frame, hopden, devil_mask,100,100)
    except:
        frame = hinhkhoihop(frame, hopden, devil_mask,100,100)
    ...
    try:
        ...
        print(count_19)
        if kckhoihop<400:
            frame = hinhkhoihop(frame, dagiac, hopden,150,100) 
        else:
            frame = hinhkhoihop(frame, dagiac, hopden,100,100) 
    except:
        frame = hinhkhoihop(frame, dagiac, hopden,100,100)
        pass
    return frame 
...
if __name__ == '__main__':
    ...
    deg = 0 
    magic_circle_cww = cv.imread('D:/Github/TIKTOK_effect_CS231_UIT/image/magic_circle_ccw.png', -1)
    magic_circle_cw = cv.imread('D:/Github/TIKTOK_effect_CS231_UIT/image/magic_circle_ccw.png', -1)
    dagiac = cv.imread('D:/Github/TIKTOK_effect_CS231_UIT/image/dagiac.jpg',-1)
    ...
    while cam.isOpened():
        ...
        frame = DK_circle_magic(frame,magic_circle_cww, magic_circle_cw, lanmarks, earthquake, count_earthquake, frame_cols, frame_rows)
        frame = DK_neon_eye(frame, lanmarks, frame_cols, frame_rows)
        frame = DK_icon_neon(frame,hopden, dagiac, devil_mask, lanmarks, frame_cols, frame_rows)
